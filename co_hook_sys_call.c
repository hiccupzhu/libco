...
{
  static __thread stCoCond_t* tls_leaky_dns_cond = NULL;
  static __thread int m_iWaitItemCnt = 0;
  int ret;

  if(tls_leaky_dns_cond == NULL) {
	tls_leaky_dns_cond = co_cond_alloc();
  }
  
  if (m_iWaitItemCnt > 0)
  	co_cond_timedwait(tls_leaky_dns_cond, -1);
  
  m_iWaitItemCnt++;
  ret = g_sys_gethostbyname_r_func(name, __result_buf, __buf, __buflen,
                                   __result, __h_errnop);
  m_iWaitItemCnt--;
  co_cond_signal(tls_leaky_dns_cond);
  return ret;
}
...
static hostbuf_wrap *get_hostbuf(pthread_once_t *routine_once, void (*__init_routine)(), pthread_key_t *routine_key)
{
	...
	hostbuf_wrap* p = (hostbuf_wrap*)co_getspecific( *routine_key );
	if( !p ) {
		p = (hostbuf_wrap*)calloc(1,sizeof(hostbuf_wrap));
		int ret = co_setspecific( *routine_key, p) ;
		if ( ret ) {
			if ( p ) {
				free(p);
...
struct hostent *co_gethostbyname(const char *name)
{
	hostbuf_wrap *hbw = get_hostbuf(&(_routine_once_hostbuf_wrap), _routine_make_key_hostbuf_wrap, &_routine_key_hostbuf_wrap);
    if (!name)
    {
        return NULL;
    }

    if (hbw->buffer && hbw->iBufferSize > 1024)
    {
        free(hbw->buffer);
        hbw->buffer = NULL;
    }
    if (!hbw->buffer)
    {
        hbw->buffer = (char*)malloc(1024);
        hbw->iBufferSize = 1024;
    }

    struct hostent *host = &hbw->host;
    ...
    {
        free(hbw->buffer);
        hbw->iBufferSize = hbw->iBufferSize * 2;
        hbw->buffer = (char*)malloc(hbw->iBufferSize);
    }
